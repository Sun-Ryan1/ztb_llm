...
class QwenTestConfig:
    ...
    def __init__(self):
        ...
        # 候选Embedding模型配置（第二阶段测试计划要求）- 仅测试bge-large-zh-v1.5
        self.embedding_models_config = {
            "bge-large-zh-v1.5": {
                "name": "BAAI/bge-large-zh-v1.5",
                "local_path": "/mnt/workspace/data/modelscope/cache/bge-large-zh-v1.5/BAAI/bge-large-zh-v1___5"
            }
        }
        ...
        # 输出目录（包含模型名称）
        self.output_dir = f"./qwen_3b_model_520qa_bge_large_chromeDB"
        ...
        # 召回率评估指标（第二阶段测试计划要求）
        self.recall_at_k_values = [1, 3, 5, 10]  # Recall@1, Recall@3, Recall@5, Recall@10
...
class ModelManager:
    ...
    def load_embedding_model(embedding_config):
        """
        加载Embedding模型
        参数: embedding_config - 包含name和local_path的配置字典
        返回: (embedding_tokenizer, embedding_model)
        """
...
def build_vector_index(embedding_models, docs: List[str], batch_size: int = 32) -> Tuple[Optional[chromadb.Collection], Optional[List[str]]]:
    ...
    Returns:
        Tuple[Optional[chromadb.Collection], Optional[List[str]]]: ChromaDB集合和有效文档列表
    """
    valid_docs = []
    ...
    if not valid_docs:
        log("❌ 无有效文档向量，无法构建ChromaDB索引", "ERROR")
        return None, None
    ...
    elapsed_time = time.time() - start_time
    log(f"✅ 共构建 {len(valid_docs)} 条有效文档的ChromaDB索引，耗时: {elapsed_time:.2f}s", "INFO")
    log(f"  集合名称: {collection_name}", "INFO")
    log(f"  向量维度: {batch_vectors.shape[1]}", "INFO")
    log(f"  倒排索引已构建，用于加速关键词检索", "INFO")
    
    return collection, valid_docs
...
def enhanced_retrieval(embedding_models, collection, docs: List[str], question: str, top_k: int = 5, similarity_threshold: float = 0.75) -> List[str]:
    ...
    for i in range(len(query_results["documents"][0])):
        ...
        # ChromaDB返回的是欧氏距离，需要转换为相似度得分
        # 这里使用归一化处理，将距离转换为0-1之间的相似度
...
def calculate_recall(retrieved_docs, relevant_docs):
    ...
    Returns:
        float: 召回率（1.0表示至少召回一个相关文档，0.0表示未召回）
    """
...
class QwenModelTestRunner:
    ...
    def run_qwen_tests(self):
        ...
        for model_key, embedding_config in self.config.embedding_models_config.items():
            ...
            for idx in range(0, total_test_cases, batch_size):
                ...
                if collection is not None:
                    ...
                    for i, test_case in enumerate(batch_test_cases):
                        ...
                        # 添加进度日志
                        if (case_idx + 1) % 10 == 0:
                            log(f"    场景2进度：{case_idx + 1}/{total_test_cases}", "INFO")
                        
                        try:
                            model_answer2, retrieved_docs = optimized_rag_inference(
                                tokenizer, llm_model, embedding_models, collection, enhanced_docs, question,
                            ...
                            recall_at_5 = calculate_recall_at_k(retrieved_docs, relevant_docs, 5)
                ...
                if processed % self.config.log_interval == 0 or processed == total_test_cases:
                    ...
                    if scenario2_results:
                        ...
                        if valid_results2:
                            ...
                            log(f"  场景2平均准确率：{avg_acc2:.4f}", "INFO")
            
            # 计算当前Embedding模型的测试结果摘要
            embedding_summary = self._calculate_summary(scenario1_results, scenario2_results)
            
            # 保存当前Embedding模型的测试结果
            embedding_results = {
                ...
                "execution_time": time.time() - embedding_start_time
            }
            
            all_embedding_results[model_key] = embedding_results
            
            # 保存当前Embedding模型的测试结果到llm_results目录
            # 确保llm_results目录存在
            llm_results_dir = os.path.join(self.output_dir, "llm_results")
            os.makedirs(llm_results_dir, exist_ok=True)
            
            # 保存场景1结果
            if embedding_results["scenario1_results"]:
                scenario1_file = os.path.join(llm_results_dir, f"{model_key}_scenario1_results.json")
            ...
            if embedding_results["scenario2_results"]:
                scenario2_file = os.path.join(llm_results_dir, f"{model_key}_scenario2_results.json")
                ...
                recall_stats = {}
                for k in [1, 3, 5, 10]:
                    recall_values = [result[f"recall@{k}"] for result in embedding_results["scenario2_results"] if f"recall@{k}" in result]
                    if recall_values:
                ...
                recall_file = os.path.join(llm_results_dir, f"{model_key}_recall_stats.json")
            ...
            # 输出当前Embedding模型的测试结果
            log(f"\n{'='*60}", "INFO")
            log(f"Embedding模型 {embedding_config['name']} 测试完成", "INFO")
            log(f"执行时间: {embedding_results['execution_time']:.2f}s", "INFO")
            log(f"\n📊 召回率指标（Recall@k）:", "INFO")
            log(f"  Recall@1: {embedding_summary['scenario2_avg_recall@1']:.4f}", "INFO")
            log(f"  Recall@3: {embedding_summary['scenario2_avg_recall@3']:.4f}", "INFO")
            log(f"  Recall@5: {embedding_summary['scenario2_avg_recall@5']:.4f}", "INFO")
            log(f"  Recall@10: {embedding_summary['scenario2_avg_recall@10']:.4f}", "INFO")
            log(f"\n📊 场景2其他指标:", "INFO")
            log(f"  平均准确率: {embedding_summary['scenario2_avg_accuracy']:.4f}", "INFO")
            log(f"  平均质量分: {embedding_summary['scenario2_avg_quality']:.4f}", "INFO")
            log(f"{'='*60}", "INFO")
        ...
        # 生成综合测试报告
        self._generate_comprehensive_report(all_embedding_results)
        
        # 记录整体测试结束时间
        overall_execution_time = time.time() - overall_start_time
        
        log(f"\n{'='*80}", "INFO")
        log(f"第二阶段Embedding模型召回率测试全部完成", "INFO")
        log(f"总执行时间: {overall_execution_time:.2f}s", "INFO")
    ...
    def _calculate_summary(self, scenario1_results, scenario2_results):
        ...
        # 场景2 recall@k统计
        scenario2_recall_at_1 = [r.get("recall@1", 0) for r in scenario2_results if "recall@1" in r]
        scenario2_recall_at_3 = [r.get("recall@3", 0) for r in scenario2_results if "recall@3" in r]
        scenario2_recall_at_5 = [r.get("recall@5", 0) for r in scenario2_results if "recall@5" in r]
        scenario2_recall_at_10 = [r.get("recall@10", 0) for r in scenario2_results if "recall@10" in r]
        ...
        return {
            ...
            "scenario2_avg_recall@1": np.mean(scenario2_recall_at_1) if scenario2_recall_at_1 else 0,
            "scenario2_avg_recall@3": np.mean(scenario2_recall_at_3) if scenario2_recall_at_3 else 0,
            "scenario2_avg_recall@5": np.mean(scenario2_recall_at_5) if scenario2_recall_at_5 else 0,
            "scenario2_avg_recall@10": np.mean(scenario2_recall_at_10) if scenario2_recall_at_10 else 0,
    ...
    def _generate_comprehensive_report(self, all_embedding_results):
        """生成所有Embedding模型的综合测试报告"""
        log(f"\n{'='*80}", "INFO")
        log(f"生成Embedding模型综合测试报告", "INFO")
        log(f"{'='*80}", "INFO")
        
        # 保存综合结果文件
        report_file = os.path.join(self.output_dir, "embedding_comparison_report.json")
        with open(report_file, "w", encoding="utf-8") as f:
            json.dump(all_embedding_results, f, ensure_ascii=False, indent=2)
        
        # 生成详细的日志报告
        log_file = os.path.join(self.output_dir, "embedding_comparison_report.log")
        with open(log_file, "w", encoding="utf-8") as f:
            f.write(f"第二阶段Embedding模型召回率测试综合报告\n")
            f.write(f"测试时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"大模型: {self.config.llm_config['llm_name']}\n")
            f.write(f"测试用例数: {self.config.max_test_cases}\n")
            f.write(f"知识库文档数: {len(self.knowledge_docs)}\n")
            f.write(f"检索top_k: {self.config.top_k_retrieval}\n")
            f.write(f"相似度阈值: {self.config.similarity_threshold}\n")
            f.write(f"\n{'='*60}\n")
            
            # 输出每个Embedding模型的详细结果
            for model_key, results in all_embedding_results.items():
                f.write(f"\nEmbedding模型: {results['embedding_config']['name']}\n")
                f.write(f"模型路径: {results['embedding_config']['local_path']}\n")
                f.write(f"执行时间: {results['execution_time']:.2f}s\n")
                f.write(f"\n召回率指标（Recall@k）:\n")
                f.write(f"  Recall@1: {results['summary']['scenario2_avg_recall@1']:.4f}\n")
                f.write(f"  Recall@3: {results['summary']['scenario2_avg_recall@3']:.4f}\n")
                f.write(f"  Recall@5: {results['summary']['scenario2_avg_recall@5']:.4f}\n")
                f.write(f"  Recall@10: {results['summary']['scenario2_avg_recall@10']:.4f}\n")
                f.write(f"\n场景2其他指标:\n")
                f.write(f"  平均准确率: {results['summary']['scenario2_avg_accuracy']:.4f}\n")
                f.write(f"  平均质量分: {results['summary']['scenario2_avg_quality']:.4f}\n")
                f.write(f"  平均回答长度: {results['summary']['scenario2_avg_answer_length']:.2f}\n")
                f.write(f"  平均检索文档数: {results['summary']['scenario2_avg_retrieved_count']:.2f}\n")
                f.write(f"\n{'='*40}\n")
            
            # 生成模型对比表格
            f.write(f"\n{'='*80}\n")
            f.write(f"Embedding模型召回率对比（Recall@k）\n")
            f.write(f"{'='*80}\n")
            f.write(f"{'模型名称':<40} {'Recall@1':>10} {'Recall@3':>10} {'Recall@5':>10} {'Recall@10':>10} {'执行时间':>10}\n")
            f.write(f"{'='*80}\n")
            
            for model_key, results in all_embedding_results.items():
                model_name = results['embedding_config']['name']
                recall1 = results['summary']['scenario2_avg_recall@1']
                recall3 = results['summary']['scenario2_avg_recall@3']
                recall5 = results['summary']['scenario2_avg_recall@5']
                recall10 = results['summary']['scenario2_avg_recall@10']
                exec_time = results['execution_time']
                f.write(f"{model_name:<40} {recall1:>10.4f} {recall3:>10.4f} {recall5:>10.4f} {recall10:>10.4f} {exec_time:>10.2f}s\n")
            
            f.write(f"{'='*80}\n")
            f.write(f"\n测试总结:\n")
            f.write(f"1. 本测试针对第二阶段测试计划要求，评估了多个Embedding模型的召回率表现\n")
            f.write(f"2. 测试使用了{self.config.max_test_cases}个测试用例和{len(self.knowledge_docs)}篇知识库文档\n")
            f.write(f"3. 召回率计算采用Recall@1、Recall@3、Recall@5和Recall@10指标\n")
            f.write(f"4. 测试结果表明不同Embedding模型在不同k值下表现存在差异\n")
            f.write(f"5. 建议根据具体应用场景选择合适的Embedding模型\n")
        
        log(f"✅ 综合报告已生成: {log_file}", "INFO")
        log(f"✅ 综合结果已保存: {report_file}", "INFO")
        
        # 输出模型对比到控制台
        log(f"\n{'='*80}", "INFO")
        log(f"Embedding模型召回率对比（Recall@k）", "INFO")
        log(f"{'='*80}", "INFO")
        log(f"{'模型名称':<40} {'Recall@1':>10} {'Recall@3':>10} {'Recall@5':>10} {'Recall@10':>10}", "INFO")
        log(f"{'='*80}", "INFO")
        ...
        for model_key, results in all_embedding_results.items():
            ...
            recall5 = results['summary']['scenario2_avg_recall@5']
            recall10 = results['summary']['scenario2_avg_recall@10']
            log(f"{model_name:<40} {recall1:>10.4f} {recall3:>10.4f} {recall5:>10.4f} {recall10:>10.4f}", "INFO")
