...
class TestConfig:
    ...
    def __init__(self):
        ...
        # 输出目录（将包含模型名称）
        self.output_dir_base = "./llama_model_selection_results"
        
        # 测试参数
        self.max_test_cases = 50  # 第一阶段可以使用较少的测试用例
        self.batch_size = 16
...
class ModelTestRunner:
    ...
    def __init__(self, config, llm_config):
        ...
        print(f"\n{'='*60}")
        print(f"Llama模型测试框架初始化完成 - {llm_config['llm_name']}")
    ...
    def run_llm_selection_tests(self):
        ...
        result_file = os.path.join(self.output_dir, "results", "test_results.json")
        with open(result_file, "w", encoding="utf-8") as f:
            json.dump(llm_results, f, ensure_ascii=False, indent=2)
        
        # 保存摘要到单独的文本文件
        summary_file = os.path.join(self.output_dir, "results", "test_summary.txt")
        with open(summary_file, "w", encoding="utf-8") as f:
            f.write(self._generate_summary_text(summary))
        
        print(f"\n✅ Llama模型 {self.llm_config['llm_name']} 测试完成")
        print(f"   场景1平均准确率: {summary['scenario1_avg_accuracy']:.4f}")
        print(f"   场景2平均准确率: {summary['scenario2_avg_accuracy']:.4f}")
        print(f"   场景2平均召回率: {summary['scenario2_avg_recall']:.4f}")
...
场景1测试结果（无提示词）:
- 平均准确率: {summary['scenario1_avg_accuracy']:.4f}
- 平均质量分: {summary['scenario1_avg_quality']:.4f}
- 平均回答长度: {summary['scenario1_avg_answer_length']:.1f}

场景2测试结果（有提示词RAG）:
- 平均准确率: {summary['scenario2_avg_accuracy']:.4f}
- 平均召回率: {summary['scenario2_avg_recall']:.4f}
- 平均质量分: {summary['scenario2_avg_quality']:.4f}
- 平均回答长度: {summary['scenario2_avg_answer_length']:.1f}
- 平均检索文档数: {summary['scenario2_avg_retrieved_count']:.1f}

测试结论:
1. 模型基本信息: {self.llm_config['llm_name']}
2. 场景1（无提示词）表现: {'良好' if summary['scenario1_avg_accuracy'] > 0.7 else '一般' if summary['scenario1_avg_accuracy'] > 0.5 else '较差'}
3. 场景2（RAG）表现: {'良好' if summary['scenario2_avg_accuracy'] > 0.7 else '一般' if summary['scenario2_avg_accuracy'] > 0.5 else '较差'}
4. RAG召回能力: {'良好' if summary['scenario2_avg_recall'] > 0.7 else '一般' if summary['scenario2_avg_recall'] > 0.5 else '较差'}

注意: 此为单个模型测试结果，用于模型选型参考。
"""
        return text
