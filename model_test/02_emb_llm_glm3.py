...
# ---------------------- 4. 配置量化参数（适配chatglm3-6b，优化显存占用） ----------------------
def get_bnb_config():
    return BitsAndBytesConfig(
        load_in_4bit=True,
        bnb_4bit_use_double_quant=True,
        bnb_4bit_quant_type="nf4",
        bnb_4bit_compute_dtype=torch.float16  # 适配chatglm3-6b的量化需求
    )

# ---------------------- 5. 加载本地chatglm3-6b模型（精准匹配指定路径） ----------------------
def load_local_models(llm_name, llm_local_path, embedding_local_path):
    print(f"正在加载本地大模型：{llm_name}")
    try:
        # chatglm3专属Tokenizer配置
        ...
        # 加载chatglm3-6b模型（严格使用本地路径，不在线下载）
...
def local_rag_inference(tokenizer, llm_model, embedding_models, index, docs, question):
    ...
    # chatglm3专属Prompt（适配招投标法规场景）
    prompt = f"""<system>你是聚焦招投标采购全流程的专业智能问答系统，需严格依据《招标投标法》《政府采购法》等法规，基于以下检索到的文档内容，准确回答问题，回答简洁明了，无需额外冗余信息。</system>
<user>文档内容：{context}
问题：{question}</user>
<assistant>"""
    ...
    if "<assistant>" in full_response:
        model_answer = full_response.split("<assistant>")[-1].strip()
    else:
...
def run_lightweight_selection_test():
    ...
    test_config = {
        ...
        "llm_local_path": "/mnt/workspace/data/modelscope/cache/ZhipuAI/chatglm3-6b",  # 图片指定大模型路径
        "embedding_local_path": "/mnt/workspace/data/modelscope/cache/qwen/Qwen3-Embedding-8B"  # 图片指定Embedding路径
    }
    ...
    test_results = []
    print("\n=====================================")
    print(f"开始 {test_config['llm_name']} 轻量级模型选型测试...")
    print("=====================================")
    ...
    for idx, case in enumerate(test_cases):
        ...
        # 招投标场景准确率：判断是否包含法规关键词或核心答案
        accuracy = 1 if (reference_answer in model_answer or 
                        any(keyword in model_answer for keyword in ["《招标投标法》", "《政府采购法》", "财库", "单一来源", "询价方式"] if keyword in reference_answer)) else 0
