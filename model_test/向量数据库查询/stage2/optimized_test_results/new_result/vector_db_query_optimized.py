    ...
    def get_vector_cache(self, query: str) -> Optional[np.ndarray]:
        ...
        with self.lock:
            ...
            if key in self.vector_cache:
                ...
                if time.time()
"""获取缓存统计（返回副本避免篡改）"""with self.lock:
...
class OptimizedVectorDBQuery:
    ...
    def _validate_params(self, query_text: str, top_k: int, min_similarity: float, where_filter: Optional[Dict]) -> None:
        """参数校验（增强版）"""
        ...
        if where_filter is not None and not isinstance(where_filter, dict):
            raise ValueError(f"where_filter必须为字典类型或None（当前: {type(where_filter)}）")
    ...
    def query(self, 
        ...
        try:
            self._validate_params(query_text, top_k, min_similarity, where_filter)
...
"""混合检索（优化权重逻辑和除零保护）"""try:
            ...
            for doc in vector_results:
                ...
                # 增加除零保护：keyword_count=0时，keyword_score=0
    ...
    def _credit_code_exact_match(self, query_text: str, top_k: int,
        ...
        try:
            ...
            for doc in exact_results:
                ...
                # 统一大小写：信用代码匹配时，content/doc_id统一转大写后再判断
...
if __name__ == "__main__":
    ...
    try:
        # 测试连接
        connection_result = queryer.test_connection()
