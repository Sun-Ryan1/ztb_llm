...
class QwenTestConfig:
    ...
    def __init__(self):
        ...
        # 输出目录（包含模型名称）
        self.output_dir = f"./qwen_model_results"
...
# ====================== 检索函数 ======================
def enhanced_retrieval(embedding_models, index, docs, question, top_k=10, similarity_threshold=0.75):
    """改进的检索函数"""
    results = []
...
def optimized_rag_inference(tokenizer, llm_model, embedding_models, index, docs, question):
    """场景2：使用专业提示词模板的RAG推理"""
    retrieved_docs = enhanced_retrieval(embedding_models, index, docs, question, top_k=10, similarity_threshold=0.75)
...
class QwenModelTestRunner:
    ...
    def run_qwen_tests(self):
        ...
        print(f"   场景2平均召回率: {qwen_results['summary']['scenario2_avg_recall']:.4f}")
