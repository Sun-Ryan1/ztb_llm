...
class QwenTestConfig:
    ...
    def __init__(self):
        ...
        # 输出目录（包含模型名称）
        self.output_dir = f"./qwen_3b_model_results_noRAG"
...
class ModelManager:
    ...
    def load_local_models(config):
        ...
        try:
            ...
            print(f"✅ BGE Embedding模型加载成功（但不使用）")
        except Exception as e:
...
def inference_with_professional_prompt(tokenizer, llm_model, question):
    """场景2：使用专业提示词模板（原场景2的提示词）"""
    # 使用原代码中的专业提示词模板，但不包含检索的上下文
    prompt = f"""# 角色定位
...
class QwenModelTestRunner:
    ...
    def run_qwen_tests(self):
        ...
        print("\n⚠️  注意：Embedding模型已加载但不会使用，仅测试LLM本身能力")
        ...
        for idx, test_case in enumerate(self.test_cases):
            ...
            if (idx + 1) % 5 == 0:
                if scenario1_results:
                    ...
                    avg_quality1 = sum([r.get("quality_score", 0) for r in scenario1_results if "quality_score" in r]) / len([r for r in scenario1_results if "quality_score" in r])
                    print(f"\n📊 当前进度：{idx+1}/{len(self.test_cases)}")
                    print(f"  场景1平均准确率：{avg_acc1:.4f} | 平均质量分：{avg_quality1:.4f}")
                ...
                if scenario2_results and any("accuracy" in r for r in scenario2_results):
                    ...
                    if valid_results2:
                        ...
                        avg_quality2 = sum([r.get("quality_score", 0) for r in valid_results2]) / len(valid_results2)
                        print(f"  场景2平均准确率：{avg_acc2:.4f} | 平均质量分：{avg_quality2:.4f}")
        ...
        result_file = os.path.join(self.output_dir, "llm_results", 
                                  f"qwen_pure_model_test_results_{datetime.now().strftime('%H%M%S')}.json")
        with open(result_file, "w", encoding="utf-8") as f:
            json.dump(qwen_results, f, ensure_ascii=False, indent=2)
        
        print(f"\n{'='*60}")
        print(f"📊 Qwen模型纯能力测试结果摘要")
        print(f"{'='*60}")
        print(f"测试用例数: {len(self.test_cases)}")
        print(f"\n场景1（无提示词直接回答）：")
        print(f"  平均准确率: {qwen_results['summary']['scenario1_avg_accuracy']:.4f}")
        print(f"  平均质量分: {qwen_results['summary']['scenario1_avg_quality']:.4f}")
        print(f"  平均回答长度: {qwen_results['summary']['scenario1_avg_answer_length']:.1f}")
        
        print(f"\n场景2（有专业提示词）：")
        print(f"  平均准确率: {qwen_results['summary']['scenario2_avg_accuracy']:.4f}")
        print(f"  平均质量分: {qwen_results['summary']['scenario2_avg_quality']:.4f}")
        print(f"  平均回答长度: {qwen_results['summary']['scenario2_avg_answer_length']:.1f}")
        
        print(f"\n准确率提升（场景2 vs 场景1）: {qwen_results['summary']['scenario2_avg_accuracy'] - qwen_results['summary']['scenario1_avg_accuracy']:+.4f}")
        print(f"质量分提升（场景2 vs 场景1）: {qwen_results['summary']['scenario2_avg_quality'] - qwen_results['summary']['scenario1_avg_quality']:+.4f}")
